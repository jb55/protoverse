...
{
  static void* lib = NULL;
  static void* glx_lib = NULL;

  static void* (*glXGetProcAddressARB)(const GLubyte*) = NULL;

  if (NULL == glx_lib) {
    glx_lib = dlopen("libGLX.so", RTLD_LAZY);
    if (!glx_lib) {
	printf("couldn't find libGLX.so\n");
	return NULL;
    }

    glXGetProcAddressARB = dlsym(glx_lib, "glXGetProcAddressARB");
    if (!glXGetProcAddressARB) {
	    printf("couldn't load glXGetProcAddressARB from libGLX.so\n");
	    return NULL;
    }
  }
